...
{
    public partial class MqMetricsService : ServiceBase
    {
        ...
        private void LoadConfiguration()
        {
            try
            {
                _stopWaitTimeout = int.Parse(_configuration.Properties["mq_stager_stop_wait_interval_secs", true]) * 1000;
                _timerInterval = int.Parse(_configuration.Properties["mq_stager_sleep_interval_millisecs", true]);
                _fatalStopCountLimit = int.Parse(_configuration.Properties["mq_stager_fatal_error_stop_count", true]);
                _errorWaitInterval = int.Parse(_configuration.Properties["mq_stager_error_wait_interval_secs", true]) * 1000;
                _timerStartTime = _configuration.Properties["mq_stager_start_time", true];
                _timerEndTime = _configuration.Properties["mq_stager_end_time", true];
            }
            catch (Exception ex)
            {
